...
truffle console
```
1. Terminal window 2 of 3, start an additional container
```bash
docker exec -it <container-name> nix-shell /app/.config/eth.nix
```
1. Run Ganache CLI
```bash
 ganache-cli --host "0.0.0.0" --networkId "CryptoStarV1" --mnemonic "rug forget problem crop act wrist helmet pilot quarter canyon crucial album"
 ```
1. Copy the mnemonic 12 words to clipboard to reuse or use the --mnemonic flag to repopulate.
1. In Metamask, click import and paste the 12 words to import the accounts running in Ganache CLI
1. Password for dev: dapppass
1. In Metamask, set the network to `http://127.0.0.1:8545` and it should pickup the 10 accounts spun up by ganache-cli
1. Terminal window 3 of 3
```bash
docker exec -it <container-name> nix-shell /app/.config/eth.nix
```
